...
class UAV(Agent):
    ...
    def _initialize_components(self):
        ...
        if self.gps is not None and ekf_cfg.get("enabled", False):
            gps_info = sensors_cfg.get("gps", {})
            
            r_pos = float(ekf_cfg.get("r_pos", gps_info.get("position_noise_std", 0.1)))
            r_vel = float(ekf_cfg.get("r_vel", gps_info.get("velocity_noise_std", 0.05)))
            ...
            print(f"[{self.name}] EKF (Observer) initialisé. accel_std={accel_std}, r_pos={r_pos}")
        else:
            self.ekf = None
    ...
    def _log_state(self, pos_true, vel_true):
        ...
        # Covariance (Pour les ellipses)
        # On logue P[0,0](x), P[1,1](y), P[0,1](xy)
        p_xx = p_yy = p_xy = np.nan

        if self.last_gps_meas:
            xg, yg, zg = self.last_gps_meas[0]
            vxg, vyg, vzg = self.last_gps_meas[1]
        
        if self.last_ekf_state:
            xe, ye, ze = self.last_ekf_state[0]
            vxe, vye, vze = self.last_ekf_state[1]
            if self.ekf:
                # Récupération de la covariance pour le logging
                P = self.ekf.P
                p_xx = P[0, 0]
                p_yy = P[1, 1]
                p_xy = P[0, 1]
        ...
        row = {
            ...
            # Ajout des covariances dans le CSV
    ...
    def think_and_act(self, setpoint: np.ndarray | None = None):
        """Boucle de contrôle basée sur la VÉRITÉ TERRAIN (Log EKF passif)."""
        ...
        pos, vel, orn_q, ang_vel = state["pos"], state["vel"], state["orn_q"], state["ang_vel"]
        ...
        if self.gps is not None:
            ...
            if self.ekf is not None:
                est_pos, est_vel = self.ekf.step(
                    m_pos, m_vel, imu_accel=imu_acc, orientation_q=orn_q
                )
        ...
        # 4. Contrôle (Basé sur 'pos' -> Vérité Terrain)
        ...
        if dist < self.pos_tolerance and np.linalg.norm(vel) < self.vel_tolerance:
            ...
            if abs(yaw_err) > self.yaw_align_threshold: ax=ay=0.0
